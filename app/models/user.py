...
def _user_doc_to_public(doc) -> dict:
    result = {
        ...
        "role_departement": doc.get("role_departement", "agent"),
...
async def create_user(user_in: UserCreate) -> dict:
    ...
    # Déterminer le rôle départemental : par défaut "agent"
    role_departement = (
        user_in.role_departement
        if hasattr(user_in, "role_departement") and user_in.role_departement
        else "agent"
    )
    ...
    if hasattr(user_in, "service_id") and user_in.service_id:
        if not dept_oid:
            raise ValueError(
                "Un service doit être assigné à un département. Veuillez d'abord sélectionner un département."
            )
...
async def authenticate_user(email: str, password: str) -> Optional[dict]:
    ...
    if not verify_password(password, user["password_hash"]):
        logger.debug(
            f"authenticate_user: Password verification failed for user: {email}"
        )
        return None
...
async def count_users_by_org(org_id: str) -> int:
    ...
    count = await db[USERS_COLLECTION].count_documents(
        {"organization_id": org_oid, "is_active": True}
    )
    return count
...
async def list_users_by_org_with_filters(org_id: str, department_id: Optional[str] = None, service_id: Optional[str] = None) -> List[dict]:
    """
    Liste les utilisateurs d'une organisation avec filtres optionnels par département et service.
    """
    db = get_database()
    try:
        org_oid = ObjectId(org_id)
    ...
    filter_dict = {"organization_id": org_oid}
    if department_id:
        try:
            dept_oid = ObjectId(department_id)
            filter_dict["department_id"] = dept_oid
        except Exception:
            return []
    ...
    if service_id:
        try:
            ...
            filter_dict["service_id"] = service_oid
        except Exception:
            return []

    cursor = db[USERS_COLLECTION].find(filter_dict)
    users = []
    async for doc in cursor:
        user_dict = _user_doc_to_public(doc)
...
async def create_user_for_org(user_in: UserCreate, org_id: str) -> dict:
    ...
    SAUF pour les gestionnaires de stock et agents DRH qui peuvent avoir leurs rôles spécifiques.
    """
    ...
    if user_in.role not in ["gestionnaire_stock", "agent_stock_drh"]:
        user_in.role = "user"
...
async def update_user(user_id: str, update_data: dict) -> dict:
    ...
    if "role_departement" in update_data:
        update_doc["role_departement"] = update_data["role_departement"]

    if "department_id" in update_data:
        if update_data["department_id"]:
            # Vérifier que le département existe
            from app.models.department import get_department_by_id
            dept = await get_department_by_id(update_data["department_id"])
            if not dept:
                raise ValueError("Département introuvable.")
            update_doc["department_id"] = ObjectId(update_data["department_id"])
        else:
            update_doc["department_id"] = None
            # Si on retire le département, retirer aussi le service
            update_doc["service_id"] = None

    if "service_id" in update_data:
        if update_data["service_id"]:
            # Vérifier que le service existe
            from app.models.department import get_service_by_id
            service = await get_service_by_id(update_data["service_id"])
            if not service:
                raise ValueError("Service introuvable.")
            # Vérifier que le service appartient au département de l'utilisateur
            if "department_id" in update_doc:
                user_dept_id = update_doc["department_id"]
            else:
                existing_dept_id = existing.get("department_id")
                user_dept_id = ObjectId(update_data["department_id"]) if "department_id" in update_data and update_data["department_id"] else existing_dept_id
            
            if user_dept_id and str(service.get("department_id")) != str(user_dept_id):
                raise ValueError("Le service n'appartient pas au département de l'utilisateur.")
            update_doc["service_id"] = ObjectId(update_data["service_id"])
        else:
            update_doc["service_id"] = None
    ...
    await db[USERS_COLLECTION].update_one({"_id": user_oid}, {"$set": update_doc})
...
async def get_directeur_by_department(department_id: str) -> Optional[dict]:
    """
    Récupère le directeur d'un département.
    """
    db = get_database()
    try:
        dept_oid = ObjectId(department_id)
    ...
    # Chercher un utilisateur avec role_departement = "directeur" dans ce département
    doc = await db[USERS_COLLECTION].find_one(
        {"department_id": dept_oid, "role_departement": "directeur", "is_active": True}
    )
...
async def get_chefs_service_by_department(department_id: str) -> List[dict]:
    """
    Récupère tous les chefs de service d'un département.
    """
    db = get_database()
    try:
        dept_oid = ObjectId(department_id)
    ...
    chefs = []
    async for doc in cursor:
        chefs.append(_user_doc_to_public(doc))
    return chefs


async def get_agents_by_service(service_id: str) -> List[dict]:
    """
    Récupère tous les agents d'un service.
    """
    db = get_database()
    try:
        service_oid = ObjectId(service_id)
    ...
    cursor = db[USERS_COLLECTION].find(
        {"service_id": service_oid, "role_departement": "agent", "is_active": True}
    )
...
async def check_user_hierarchy_permission(
    user_id: str, target_department_id: str
) -> bool:
    """
    Vérifie si un utilisateur peut valider les demandes d'un département.
    Un directeur peut valider les demandes de son département.
    """
    db = get_database()
    try:
        user_oid = ObjectId(user_id)
        dept_oid = ObjectId(target_department_id)
    except Exception:
        return False
